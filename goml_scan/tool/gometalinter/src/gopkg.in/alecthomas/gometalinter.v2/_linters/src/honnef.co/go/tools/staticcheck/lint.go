	...
	fn := func(node ast.Node) bool {
		...
		for _, arg := range call.Args {
			...
			if isName(j, arg, "os.Kill") || isName(j, arg, "syscall.SIGKILL") {
				...
				j.Errorf(arg, "staticcheck/signal->%s 不能被捕获（是syscall.SIGTERM吗？）", j.Render(arg))
			}
			if isName(j, arg, "syscall.SIGSTOP") {
				//j.Errorf(arg, "%s signal cannot be trapped", j.Render(arg))
				j.Errorf(arg, "staticcheck/signal->%s 信号不能被捕获", j.Render(arg))
			}
		}
		return true
	}
...
func (c *Checker) CheckTimeSleepConstant(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call.Args[0], "staticcheck/sleep->睡眠 %d（ns）很可能是一个bug。如果不是的话请明确下:%s", n, recommendation)
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
...
func (c *Checker) CheckWaitgroupAdd(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if fn.FullName() == "(*sync.WaitGroup).Add" {
			//j.Errorf(sel, "should call %s before starting the goroutine to avoid a race",j.Render(stmt))
			j.Errorf(sel, "staticcheck/routine->启动goroutine前应该先调用%s，以避免竞争",j.Render(stmt))
		}
		return true
	}
	for _, f := range j.Program.Files {
...
func (c *Checker) CheckInfiniteEmptyLoop(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(loop, "staticcheck/loop->这个循环会spin，导致100%CPU的使用")
		if loop.Cond != nil {
			//j.Errorf(loop, "loop condition never changes or has a race condition")
			j.Errorf(loop, "staticcheck/loop->循环条件一直没有变化，或者存在竞争条件")
		}
...
func (c *Checker) CheckDeferInInfiniteLoop(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		for _, stmt := range defers {
			...
			j.Errorf(stmt, "staticcheck/loop->无限循环中的defers永远不会运行")
		}
		return true
	}
	for _, f := range j.Program.Files {
	...
	fn := func(node ast.Node) bool {
		...
		fn2 := func(node ast.Node) bool {
			...
			case *ast.DeferStmt:
				...
				j.Errorf(stmt, "staticcheck/loop->在range循环中，defers不会运行，除非通道被关闭")
			case *ast.FuncLit:
				// Don't look into function bodies
				return false
			}
...
func (c *Checker) CheckTestMainExit(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if !callsExit && callsRun {
			...
			j.Errorf(node, "staticcheck/exit->TestMain应该调用os.Exit来设置退出码")
		}
		return true
	}
	for _, f := range j.Program.Files {
...
func (c *Checker) CheckExec(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call.Args[0], "staticcheck/exec->exec.Command的第一个参数看起来是shell命令，但是缺少了程序名或者路径")
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
	...
	fn := func(node ast.Node) bool {
		...
		for _, c := range sel.Body.List {
			if comm, ok := c.(*ast.CommClause); ok && comm.Comm == nil && len(comm.Body) == 0 {
				...
				j.Errorf(comm, "staticcheck/loop->在for+select循环中不应该存在空的default case，会导致循环spin")
			}
		}
		return true
	}
...
func (c *Checker) CheckLhsRhsIdentical(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(op, "staticcheck/exp->%s操作符两边存在相同的表达式", op.Op)
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
		...
		for _, stmt := range body.List {
			...
			for _, body := range blocks {
				...
				for _, last := range lasts {
					...
					j.Errorf(branch, "staticcheck/break->break语句没有效果，是希望跳出外部循环吗？")
				}
			}
		}
		return true
...
func (c *Checker) CheckUnsafePrintf(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call.Args[0], "staticcheck/args->该函数类似print风格的函数，有第一个动态参数，但没有其他更多的参数。应该使用print风格的函数。")
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
...
func (c *Checker) CheckEarlyDefer(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		for i, stmt := range block.List {
			...
			j.Errorf(def, "staticcheck/defer->defer %s 前，应该先检查返回的错误码", j.Render(def.Call))
		}
		return true
	}
	for _, f := range j.Program.Files {
	...
	fn := func(node ast.Node) bool {
		...
		for i := range block.List[:len(block.List)-1] {
			...
			if (method1 == "Lock" && method2 == "Unlock") ||
				...
				j.Errorf(block.List[i+1], "staticcheck/resource->空的临界区（critical section)")
			}
		}
		return true
	}
	...
	fn := func(node ast.Node) bool {
		if unary, ok := node.(*ast.UnaryExpr); ok {
			if star, ok := unary.X.(*ast.StarExpr); ok && unary.Op == token.AND {
				...
				if !ok || !cgoIdent.MatchString(ident.Name) {
					...
					j.Errorf(unary, "staticcheck/simple->&*x 可以简化为 x。这个用法不会复制x。")
				}
			}
		}
		...
		if star, ok := node.(*ast.StarExpr); ok {
			if unary, ok := star.X.(*ast.UnaryExpr); ok && unary.Op == token.AND {
				...
				j.Errorf(star, "staticcheck/simple->&*x 可以简化为 x。这个用法不会复制x。")
			}
		}
		return true
	}
		...
		for _, b := range ssafn.Blocks {
			for _, ins := range b.Instrs {
				...
				if r1.Length.Intersection(r2.Length).Empty() {
					...
					j.Errorf(binop, "staticcheck/compare->比较两个不同长度的字符串是否相等，会永远返回false")
				}
			}
		}
	}
...
func (c *Checker) CheckCanonicalHeaderKey(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(op, "staticcheck/httpheader->在http.Header里的keys都是规范化的，然而%q并不符合规范；修改该常量或者使用http.CanonicalHeaderKey", s)
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
...
func (c *Checker) CheckBenchmarkN(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(assign, "staticcheck/assign->不应该赋值给%s", j.Render(sel))
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
			...
			if len(assign.Lhs) > 1 && len(assign.Rhs) == 1 {
				...
				for _, ref := range *refs {
					...
					if len(lint.FilterDebug(*exrefs)) == 0 {
						...
						j.Errorf(lhs, "staticcheck/unused->这个变量%s的值不会被使用", lhs)
					}
				}
				return true
			}
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				if trues == 0 || trues == len(xs)*len(ys) {
					//j.Errorf(ssabinop, "binary expression is always %t for all possible values (%s %s %s)", b, xs, ssabinop.Op, ys)
					j.Errorf(ssabinop, "staticcheck/exp->二元表达式永远是 %t，对于所有可能的值（%s %s %s）", b, xs, ssabinop.Op, ys)
				}
			}
		}
	}
	...
	for _, ssafn := range j.Program.InitialFunctions {
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				j.Errorf(mu, "staticcheck/assign->赋值给nil map")
			}
		}
	}
}
...
func (c *Checker) CheckUnsignedComparison(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		case token.GEQ:
			...
			j.Errorf(expr, "staticcheck/compare->无符号数永远 >= 0")
		case token.LSS:
			//j.Errorf(expr, "unsigned values are never < 0")
			j.Errorf(expr, "staticcheck/compare->无符号数不可能 < 0")
		}
		return true
	}
	for _, f := range j.Program.Files {
...
func (c *Checker) CheckLoopCondition(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(cond, "staticcheck/loop->循环变量没有变化")
		...
		for _, field := range typ.Params.List {
			for _, arg := range field.Names {
				...
				if assigned {
					...
					j.Errorf(arg, "staticcheck/args->参数%s在使用前被覆写了", arg)
				}
			}
		}
		return true
	...
	fn := func(node ast.Node) bool {
		...
		ast.Inspect(body, func(node ast.Node) bool {
			...
			if unconditionalExit != nil {
				...
				j.Errorf(unconditionalExit, "staticcheck/loop->外层循环无条件终止")
			}
			return true
		})
		return true
...
func (c *Checker) CheckSeeker(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call, "staticcheck/args->io.Seeker的第一个参数是偏移值offset，但是使用了io.Seek*常量")
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				if !isUsed {
					...
					j.Errorf(ins, "staticcheck/append->append的结果不会被使用，除非在其他的appends里")
				}
			}
		}
	}
			...
			for _, ins := range block.Instrs {
				...
				for _, block := range fn.Blocks {
					for _, ins := range block.Instrs {
						...
						j.Errorf(gostmt, "staticcheck/goroutine->goroutine调用T.%s，作为测试test必须在相同的goroutine里调用", name)
					}
				}
			}
		}
		...
		for _, edge := range node.Out {
			...
			for _, b := range mc.Bindings {
				if b == v {
					...
					j.Errorf(edge.Site, "staticcheck/finalizer->the finalizer closes over the object, preventing the finalizer from ever running (at %s)", pos)
				}
			}
		}
	}
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				if idxr.Lower.Cmp(sr.Length.Upper) >= 0 {
					...
					j.Errorf(ia, "staticcheck/overrun->index索引超出界限（bounds）")
				}
			}
		}
	}
	...
	for _, ssafn := range j.Program.InitialFunctions {
		for _, block := range ssafn.Blocks {
			...
			for i, ins := range instrs[:len(instrs)-1] {
				...
				j.Errorf(nins, "staticcheck/defer->刚好在lock之后defer %s，你是想defer %s吗？", name, alt)
			}
		}
	}
}
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				if isNaN(ins.X) || isNaN(ins.Y) {
					...
					j.Errorf(ins, "staticcheck/compare->没有数值等于NaN，即使Nan本身")
				}
			}
		}
	}
...
func (c *Checker) CheckInfiniteRecursion(j *lint.Job) {
	for _, ssafn := range j.Program.InitialFunctions {
		...
		for _, edge := range node.Out {
			...
			j.Errorf(edge.Site, "staticcheck/infinite->调用了无限递归")
		}
	}
}
	...
	for _, ssafn := range j.Program.InitialFunctions {
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				j.Errorf(call, "staticcheck/timeticker->使用time.Tick在某些场景下会泄漏。可以考虑在endless function中使用，或者使用time.NewTicker。")
			}
		}
	}
}
...
func (c *Checker) CheckDoubleNegation(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(unary1, "staticcheck/boolean->对于boolean类型的双重否定是没有效果的，是笔误吗？")
		return true
	}
	for _, f := range j.Program.Files {
		ast.Inspect(f, fn)
	...
	fn := func(node ast.Node) bool {
		...
		for _, cond := range conds {
			...
			if counts[s] == 2 {
				...
				j.Errorf(cond, "staticcheck/condition->这个条件多次出现在if/else if链中")
			}
		}
		return true
	}
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				case token.AND:
					...
					j.Errorf(ins, "staticcheck/compare->x & 0的结果永远等于 0")
				case token.OR, token.XOR:
					//j.Errorf(ins, "x %s 0 always equals x", ins.Op)
					j.Errorf(ins, "staticcheck/compare->x %s 0的结果永远等于x", ins.Op)
				}
			}
		}
	}
	...
	fn := func(node ast.Node) bool {
		...
		for _, i := range args {
			...
			if len(lit.Value) == 3 &&
				...
				j.Errorf(call.Args[i], "staticcheck/simple->文件模式 %s 计算结果是 %#o；是否应该是 0%s", lit.Value, v, lit.Value)
			}
		}
		return true
	}
		...
		for _, b := range ssafn.Blocks {
			for _, ins := range b.Instrs {
				...
				if c.funcDescs.Get(callee).Pure && !c.funcDescs.Get(callee).Stub {
					...
					j.Errorf(ins, "staticcheck/return->%s 是一个纯函数（pure function），但是它的返回值被忽略了", callee.Name())
					continue
				}
			}
		}
...
func (c *Checker) CheckDeprecated(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if ok, alt := c.isDeprecated(j, sel.Sel); ok {
			...
			j.Errorf(sel, "staticcheck/deprecated->%s 已经废弃：%s", j.Render(sel), alt)
			return true
		}
		return true
	}
		...
		for _, block := range ssafn.Blocks {
			for _, ins := range block.Instrs {
				...
				case *ssa.Store:
					...
					j.Errorf(ins, "staticcheck/iowriter->io.Writer.Write 不能修改所提供的buffer缓冲区，即使是临时性的")
				case *ssa.Call:
...
func (c *Checker) CheckEmptyBranch(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if ifstmt.Else != nil {
			...
			j.Errorf(ifstmt.Else, "staticcheck/emptybranch->空的分支")
		}
		if len(ifstmt.Body.List) != 0 {
			return true
		}
		//j.Errorf(ifstmt, "empty branch")
		j.Errorf(ifstmt, "staticcheck/emptybranch->空的分支")
		return true
	}
	...
	for _, fn := range j.Program.InitialFunctions {
		...
		insLoop:
			for _, ins := range b.Instrs {
				...
				j.Errorf(conv, "staticcheck/efficient->m[string(key)] 可能比 k := string(key); m[k] 更加高效")
			}
		}
	}
}
	...
	fn := func(node ast.Node) bool {
		...
		for i, stmt := range assign.Lhs {
			...
			if rlh == rrh {
				...
				j.Errorf(assign, "staticcheck/assign->%s 自赋值为 %s", rrh, rlh)
			}
		}
		return true
	}
