...
func (c *Checker) LintSingleCaseSelect(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		case *ast.ForStmt:
			...
			j.Errorf(node, "gosimple/for->应该使用for range，而不是 for select{}")
		case *ast.SelectStmt:
			if _, ok := seen[v]; ok {
				return true
			}
			...
			j.Errorf(node, "gosimple/sendrecv->应该使用简单通道的send/receive，而不是select")
			return true
		}
		return true
	}
...
func (c *Checker) LintLoopCopy(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(loop, "gosimple/copy->应该使用copy()而不是循环")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintIfBoolCmp(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(expr, "gosimple/boolcmp->应该省略bool常量的比较，可以简化为：%s", r)
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintBytesBufferConversions(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if typ == types.Universe.Lookup("string").Type() && j.IsCallToAST(call.Args[0], "(*bytes.Buffer).Bytes") {
			...
			j.Errorf(call, "gosimple/usageadvice->应该使用%v.String()方法而不是%v ", j.Render(sel.X), j.Render(call))
			...
			j.Errorf(call, "gosimple/usageadvice->应该使用%v.Bytes()方法而不是%v", j.Render(sel.X), j.Render(call))
		}
...
func (c *Checker) LintStringsContains(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(node, "gosimple/usageadvice->应该使用 %s%s.%s(%s)", prefix, pkgIdent.Name, newFunc, j.RenderArgs(call.Args))
...
func (c *Checker) LintBytesCompare(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(node, "gosimple/usageadvice->应该使用%sbytes.Equal(%s) ", prefix, args)
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintForTrue(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(loop, "gosimple/usageadvice->应该使用 for {} 而不是 for true {}")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintRegexpRaw(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call, "gosimple/usageadvice->应该使用原生字符串 (`...`)及regexp.%s避免转义两次", sel.Sel.Name)
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintIfReturn(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(n1, "gosimple/usageadvice->应该使用 'return <expr>'，而不是 'if <expr> { return <bool> }; return <bool>'")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintRedundantNilCheckWithLen(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(expr, "gosimple/usageadvice->应该省略nil检查；%s执行len()定义为0", nilType)
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintSlicing(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(n, "gosimple/usageadvice->使用slice时应该省略第二个index，s[a:len(s)] 等效于 s[a:]")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintLoopAppend(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(loop, "gosimple/usageadvice->应该使用%s = append(%s, %s...) 来替换循环",
			j.Render(stmt.Lhs[0]), j.Render(call.Args[0]), j.Render(loop.X))
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
...
func (c *Checker) LintTimeSince(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call, "gosimple/usageadvice->应该使用time.Since，而不是time.Now().Sub")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintTimeUntil(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(call, "gosimple/usageadvice->应该使用time.Util，而不是t.Sub(time.Now())")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
	...
	fn1 := func(node ast.Node) bool {
		...
		outer:
			for i, stmt := range block.List {
				...
				j.Errorf(ifs, "gosimple/usageadvice->'if %s != nil { return %s }; return %s' 可以简化为 'return %s'",
					j.Render(expr.X), j.RenderArgs(ret1.Results),
					j.RenderArgs(ret2.Results), j.RenderArgs(ret1.Results))
			}
			return true
...
func (c *Checker) LintUnnecessaryBlank(j *lint.Job) {
	fn1 := func(node ast.Node) {
		...
		j.Errorf(assign, "gosimple/usageadvice->应该写 %s 而不是 %s", j.Render(&cp), j.Render(assign))
	}
	...
	fn2 := func(node ast.Node) {
		...
		for i, lh := range stmt.Lhs {
			...
			j.Errorf(lh, "gosimple/usageadvice->''_ = <-ch' 可以简化为 '<-ch'")
		}
	}
	...
	fn3 := func(node ast.Node) {
		...
		if lint.IsBlank(rs.Key) && (rs.Value == nil || lint.IsBlank(rs.Value)) {
			...
			j.Errorf(rs.Key, "gosimple/usageadvice->应该省略range的值；这个循环等效与 `for range ...`")
		}
	}
...
func (c *Checker) LintSimplerStructConversion(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(node, "gosimple/usageadvice->应该转换 %s(类型为%s）到%s，而不是使用struct结构",
			ident.Name, typ2.Obj().Name(), typ1.Obj().Name())
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
...
func (c *Checker) LintTrim(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(ifstmt, "gosimple/usageadvice->应该使用无条件的 %s.%s 来替换这个if语句", pkg, replacement)
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintLoopSlide(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		*/
		...
		j.Errorf(loop, "gosimple/usageadvice->应该使用copy(%s[:%s], %s[%s:])", j.Render(bs1), j.Render(biny), j.Render(bs1), j.Render(add1))
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
	...
	fn := func(node ast.Node) bool {
		...
		case 2:
			...
			if lint.IsZero(call.Args[1]) {
				...
				j.Errorf(call.Args[1], "gosimple/usageadvice->应该使用make(%s)", j.Render(call.Args[0]))
			}
		case 3:
			// make(T, len, cap)
			if j.Render(call.Args[1]) == j.Render(call.Args[2]) {
				...
				j.Errorf(call.Args[1], "gosimple/usageadvice->应该使用make(%s, %s)", j.Render(call.Args[0]), j.Render(call.Args[1]))
			}
		}
		return false
	}
...
func (c *Checker) LintAssertNotNil(j *lint.Job) {
	...
	fn := func(node ast.Node) bool {
		...
		j.Errorf(ifstmt, "gosimple/usageadvice->当 %s 为true时，%s不可能为nil ", j.Render(assignIdent), j.Render(assertIdent))
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintDeclareAssign(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		for i, stmt := range block.List[:len(block.List)-1] {
			...
			j.Errorf(decl, "gosimple/usageadvice->应该在新的一行里合并变量的声明和赋值")
		}
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
...
func (c *Checker) LintRedundantBreak(j *lint.Job) {
	fn1 := func(node ast.Node) {
		...
		j.Errorf(branch, "gosimple/usageadvice->多余的break语句")
		return
	}
	fn2 := func(node ast.Node) {
		var ret *ast.FieldList
		...
		j.Errorf(rst, "gosimple/usageadvice->多余的return语句")
	}
	fn := func(node ast.Node) bool {
		fn1(node)
		fn2(node)
...
func (c *Checker) LintRedundantSprintf(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		if c.Implements(j, typ, "fmt.Stringer") {
			...
			j.Errorf(call, "gosimple/usageadvice->应该使用String()，而不是fmt.Sprintf")
			return true
		}
		...
		if typ.Underlying() == types.Universe.Lookup("string").Type() {
			if typ == types.Universe.Lookup("string").Type() {
				...
				j.Errorf(call, "gosimple/usageadvice->参数已经是字符串，没有必要使用fmt.Sprintf")
			} else {
				//j.Errorf(call, "the argument's underlying type is a string, should use a simple conversion instead of fmt.Sprintf")
				j.Errorf(call, "gosimple/usageadvice->参数的基础类型是字符串，应该使用简单的转换而不是使用fmt.Sprintf")
			}
		}
		return true
	}
...
func (c *Checker) LintErrorsNewSprintf(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		j.Errorf(node, "gosimple/usageadvice->应该使用fmt.Errorf(...)，而不是errors.New(fmt.Sprintf(...))")
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
		ast.Inspect(f, fn)
...
func (c *Checker) LintNilCheckAroundRange(j *lint.Job) {
	fn := func(node ast.Node) bool {
		...
		case *types.Slice, *types.Map:
			...
			j.Errorf(node, "gosimple/usageadvice->不必要的nil检查")
		}
		return true
	}
	for _, f := range c.filterGenerated(j.Program.Files) {
