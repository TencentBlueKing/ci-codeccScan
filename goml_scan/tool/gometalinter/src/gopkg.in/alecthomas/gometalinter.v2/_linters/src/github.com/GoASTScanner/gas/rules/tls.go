		...
		case "InsecureSkipVerify":
			if node, ok := n.Value.(*ast.Ident); ok {
				if node.Name != "false" {
					...
					return gas.NewIssue(c, n, "gas/tls->InsecureSkipVerify设为true", gas.High, gas.High)
				}
			} else {
				...
				return gas.NewIssue(c, n, "gas/tls->InsecureSkipVerify可能为true", gas.High, gas.Low)
			}
		...
		case "PreferServerCipherSuites":
			if node, ok := n.Value.(*ast.Ident); ok {
				if node.Name == "false" {
					...
					return gas.NewIssue(c, n, "gas/tls->PreferServerCipherSuites设为false", gas.Medium, gas.High)
				}
			} else {
				// TODO(tk): symbol tab look up to get the actual value
				//return gas.NewIssue(c, n, "TLS PreferServerCipherSuites may be false.", gas.Medium, gas.Low)
				return gas.NewIssue(c, n, "gas/tls->PreferServerCipherSuites可能为false", gas.Medium, gas.Low)
			}
		...
		case "MinVersion":
			if ival, ierr := gas.GetInt(n.Value); ierr == nil {
				if (int16)(ival) < t.MinVersion {
					...
					return gas.NewIssue(c, n, "gas/tls->MinVersion太低", gas.High, gas.High)
				}
				// TODO(tk): symbol tab look up to get the actual value
				//return gas.NewIssue(c, n, "TLS MinVersion may be too low.", gas.High, gas.Low)
				return gas.NewIssue(c, n, "gas/tls->MinVersion可能太低", gas.High, gas.Low)
			}
		...
		case "MaxVersion":
			if ival, ierr := gas.GetInt(n.Value); ierr == nil {
				if (int16)(ival) < t.MaxVersion {
					...
					return gas.NewIssue(c, n, "gas/tls->MaxVersion太低", gas.High, gas.High)
				}
				// TODO(tk): symbol tab look up to get the actual value
				//return gas.NewIssue(c, n, "TLS MaxVersion may be too low.", gas.High, gas.Low)
				return gas.NewIssue(c, n, "gas/tls->MaxVersion可能太低", gas.High, gas.Low)
			}
