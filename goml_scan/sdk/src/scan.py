...
go_build_faild_skip_linter = ['maligned', 'varcheck', 'structcheck', 'unparam', 'errcheck', 'gotype', 'interfacer',
                              'unconvert']
...
def scan(filename, third_rules, skip_path_list):
    ...
    if os.path.isdir(filename):
        if not filename.endswith("/"):
            filename = filename + "/"
        cmd = "gometalinter %s... --sort=path --deadline=60m --format={{.Path.Abs}}'->'{{.Line}}'->'{{.Linter}}'->'{{.Message}} --enable-all %s %s %s %s --exclude=vendor -j 2" % \
              (filename, skip_option, config.checker_options, default_disable_linter, build_failed_disable_linter)
    else:
        cmd = "gometalinter %s --sort=path --deadline=60m --format={{.Path.Abs}}'->'{{.Line}}'->'{{.Linter}}'->'{{.Message}} --enable-all %s %s %s %s --exclude=vendor -j 2" % \
              (filename, skip_option, config.checker_options, default_disable_linter, build_failed_disable_linter)
    print(cmd)
