...
def GetAllExtensions():
    return set(['c','C','c++','cc','CPP','cpp','cxx','pcc','H','h','hh','hpp','hxx'])
...
def scan(filename, config_path, third_rules, skip_path_list):
    ...
    if os.path.isdir(filename):
        cmd = "python %s/../../tool/cpplint_external.py --output=codecc --cfg=%s --third-rule-path=%s --recursive \"%s\" " % (config.current_path, config.rule_config_file, third_rules, filename)
    else:
        cmd = "python %s/../../tool/cpplint_external.py --output=codecc --cfg=%s --third-rule-path=%s \"%s\" " % (config.current_path, config.rule_config_file, third_rules, filename)
    ...
    for elem in cmd_result:
        ...
        if os.path.isfile(error_smg[0]) and check_path_match_skip(error_smg[0], skip_path_list) and str(error_smg[1]) != 'None':
            defect = {}
