...
def scan(filename, config_path, third_rules, skip_path_list):
    file_defects = []
    cmd_result = []
    cmd = "php -d memory_limit=\"-1\" %s/../../tool/phpcs.phar --standard=%s --extensions=php --report=json \"%s\" " % (config.current_path, config.rule_config_file, filename)
    ...
    try:
        ...
        if 'files' in data:
            files_infos = data['files']
            for filePath, error_infos in files_infos.items():
                if not check_path_match_skip(filePath, skip_path_list):
                    continue
                if 'messages' in error_infos:
                    for error in error_infos['messages']:
                        defect = {}
                        defect['filePath'] = filePath
                        defect['checkerName'] = error['source']
                        defect['description'] = error['message']
                        defect['line'] = error['line']
                        if defect != {}:
                            file_defects.append(defect)
    except:
        print('pase the '+filename+' failed')
